...
const SandboxPanel = ({ className }: SandboxPanelProps) => {
  ...
  return (
    <div className={cn("flex flex-col gap-5", className)}>
      <section className="space-y-5 rounded-3xl border border-border bg-secondary/40 p-6 shadow-glow">
        <div>
          <label className="text-xs font-semibold uppercase tracking-[0.2em] text-muted-foreground/80">API Provider</label>
          <select
            value={providerSlug}
        ...
        <div>
          ...
          <textarea
            ...
            rows={provider.method === "GET" ? 4 : 8}
            className="mt-2 w-full rounded-xl border border-border bg-background px-3 py-3 text-sm shadow-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"
            placeholder="Enter JSON payload or query string"
          />
          <p className="mt-2 text-xs text-muted-foreground">
            Include your wallet-backed <code className="font-mono text-[11px]">x402-session</code> header when testing from your
            client.
          </p>
        </div>
      ...
      <section className="rounded-3xl border border-border bg-secondary/30 p-6 shadow-inner">
        ...
        <pre className="mt-4 h-[360px] overflow-auto rounded-2xl bg-black/90 p-5 text-sm leading-relaxed text-white">
